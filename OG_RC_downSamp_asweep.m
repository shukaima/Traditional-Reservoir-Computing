...
Nin=1;
Nout=2;
alist = [0.001 0.01 0.05 0.1 0.2 0.4 0.6 0.8];
for iN = 1:length(alist)
...
%%
plot(alist,RMSE_testy,'-x','DisplayName','RMSE_y'); hold on
plot(alist,RMSE_testz,'-x','DisplayName','RMSE_z'); hold on
legend
xlabel('\alpha_{RC}');
ylabel('RMSE');
title(['Software RC, \beta = ' num2str(bt) ', N_{node} = ' num2str(Nres)]);
box on
